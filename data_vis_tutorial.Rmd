...
```{r}
ggplot(data=vtl_data) + 
  geom_point(mapping = aes(x = age_yrs, y=vtl, shape=gender), color='green') 
```
...
ggplot(data=., aes(x=age_yrs, y=vtl)) + # notice that I'm now mapping my "main" variables at the beginning of my plot
  ...
  geom_jitter(aes(color=gender, shape=gender, width = .05), alpha=0.75, size=2) + # geom point was nice but I'd like to be able to see the individual points
...
ggplot(data=., mapping = aes(x=deltaf, y=vtl)) + # again, now we don't have to specify in every geom object
  ...
  geom_smooth(method="lm", color="gray55") +  # we have a new geom object here; what happens when you change the method?
...
hist_plot <-  ggplot(hist_plot_data, aes(x=frequency, fill=formant)) + # note the absence of a variable mapped to 'y'
  
  geom_density(alpha=.75) + # what happens when we plot geom_histogram instead?
