...
#output: html_notebook # faster to generate; won't produce tables or figures
#output: pdf_document
output:
...
* Generates attractive reports in .tex, .pptx, .pdf, .html, and .docx formats
...
* Give your chunk a name to easily find it in large scripts
...
library('lme4')
library('stargazer')
library('webshot') # to render html output in word docs and powerpoints
...
* By generating a .tex or .docx file from your markdown, you have an editable table to import into your paper
...
kable(., 
      ...
      booktabs=T, 
      row.names = FALSE,
      col.names = c("Age", "F1 mean (SD)", "F2 mean (SD) making this name really really really long")) %>%
...
We can write in multiple paragraphs, in fact, just by leaving a line between them in the document and two spaces after the previous line.
...
You could also reference components of your statistical tests like reporting that there is a positive correlation between age and vocal tract length (whew, sanity check!) (r=`r round(vtl_cor[,1],2)`). This means that even if (ahem) you forget a participant or two in your first analysis, or you run more participants, you don't have to rewrite your entire results section to include the new participants. It's updated automatically every time you compile your script. 
...
# Efficiently reporting on models

Now that you know how to create attractive, interactive tables, and you know how to report statistics in-line, we can bring those skills together to report on a model summary. 

* Try customizing the model summary table
   + Adding a header
   + Fitting a model with different predictors
   + Displaying *multiple* models side-by-side (step-wise modeling)
   + Knit the whole project together to see what the summary looks like


```{r, create model summary table, comment=NA}
# there are several different packages available to create model summaries
# we'll use Stargazer

# first, get the data in order and fit a model
model_fit <- data %>%
  mutate(age_yrs=factor(age_yrs, ordered = FALSE),
         age_yrs=relevel(age_yrs, ref='4')) %>%
  lmer(f2_midpt_med_clean ~ age_yrs + Phone + (1|spkr), data=.)

stargazer(model_fit, 
            header=FALSE, 
            dep.var.caption = "", 
            dep.var.labels.include = FALSE,  
            type = "latex", # I specify latex because I sew my markdown results into latex for paper submissison
            star.cutoffs=c(0.05,0.01,0.001), 
            star.char = c("*", "**", "***"),
            title="Model predicting F2 by age and phone",  
            digits = 2, 
            ci = TRUE, 
            style = "all",
            order=c(10,2,3,4,5,6,1,7,8,9), # reorder the parameters of the model
            covariate.labels = c("Intercept", "5-year-olds", "6-year-olds", "7-year-olds", 
                                 "8-year-olds", "9-year-olds", "10-year-olds", "Adults", "Phone:i", "Phone:u"))


```

```{r, side-by-side models, comment=NA}
# fit a couple more models and try to display three simultaneously 
# within minimal space wasted, you can demonstrate how the addition of parameters improves your model fit 
base_model <- data %>%
  mutate(age_yrs=factor(age_yrs, ordered = FALSE),
         age_yrs=relevel(age_yrs, ref='4')) %>%
  lmer(f2_midpt_med_clean ~ (1|spkr), data=.)

phone_model <- data %>%
  mutate(age_yrs=factor(age_yrs, ordered = FALSE),
         age_yrs=relevel(age_yrs, ref='4')) %>%
  lmer(f2_midpt_med_clean ~ Phone + (1|spkr), data=.)

# an easy way to display multiple models
# this particular set results in a fair amount of white
# space because our age variable has so many levels
# but this is an especially good idea if you have many
# continuous variables that you fit sequentially
# in your model and you want to demonstrate your model
# fitting procedure (in supplementary materials or to
# show your advisor, for ex.)
stargazer(base_model, phone_model, model_fit, 
            header=FALSE, 
            dep.var.caption = "", 
            dep.var.labels.include = FALSE,  
            type = "latex",
            star.cutoffs=c(0.05,0.01,0.001), 
            star.char = c("*", "**", "***"),
            title="Stepwise modeling of F2 by age and phone",  
            digits = 2, 
            ci = TRUE, 
            style = "all",
            order=c(10,2,3,4,5,6,1,7,8,9), # reorder the parameters of the model
...
Finally, you have the option to sew snippets of code from other languages right into your script! Even if you're not super familiar with shell scripting or Python, this could still be helpful. Imagine that a colleague sends you a Python script, or you find a few lines of Shell code online. Now instead of translating it into R, you could copy it verbatim. 
...
```{bash, a bash example, warning=FALSE}
# in your terminal below, you will need to install 'sox' in order to run this line of code
# either 'brew install sox' or 'pip install sox' (only if you've already install homebrew)
